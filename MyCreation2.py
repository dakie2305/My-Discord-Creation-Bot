...
from Handling.MiniGame.RandomQuizz.RandomQuizzView import (
    RandomQuizzView,
    random_quizzes,
)
...
async def synccre2(ctx):
    if ctx.author.id == CustomFunctions.user_darkie["user_id"]:
        ...
        await ctx.send(
            f"Đã đồng bộ thêm {len(fmt)} các slash commands vào Server {ctx.guild}"
        )
    else:
        await ctx.send("Có phải là Darkie đâu mà dùng lệnh này?")
...
async def global_sync_creation_2(ctx):
    if ctx.author.id == CustomFunctions.user_darkie["user_id"]:
        ...
        await ctx.send(
            f"Đã đồng bộ hết {len(fmt)} slash commands của Creation 2 vào toàn bộ server hiện hành!"
        )
    else:
        await ctx.send("Có phải là Darkie đâu mà dùng lệnh này?")
...
async def guild_extra_info(ctx):
    ...
    if check_exist:
        ...
        data = GuildExtraInfo(
            guild_id=ctx.guild.id, guild_name=ctx.guild.name, allowed_ai_bot=False
        )
        db.insert_guild_extra_info(data)
        await ctx.send(
            f"Lưu thành công thông tin Guild Extra Info về server này.", ephemeral=True
        )
...
@bot.tree.command(
    name="random_ai_talk",
    description="Bật/tắt chế độ cho phép bot lâu lâu trò chuyện trong channel này.",
)
async def random_ai_talk(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    if (
        interaction.user.id != 315835396305059840
        and interaction.user.id != interaction.guild.owner_id
    ):
        await interaction.followup.send(
            f"Chỉ chủ Server mới dùng lệnh này.", ephemeral=False
        )
        return
    ...
    if check_exist:
        ...
        db.update_guild_extra_info(
            guild_id=interaction.guild.id, update_data=data_updated
        )
        if add:
            await interaction.followup.send(
                f"Bot lâu lâu sẽ nói chuyện trong channel này.", ephemeral=True
            )
        else:
            await interaction.followup.send(
                f"Bot sẽ không còn nói chuyện trong channel này nữa.", ephemeral=True
            )
    else:
        list_channels_ai_talk.append(called_channel.id)
        data = GuildExtraInfo(
            ...
            allowed_ai_bot=True,
            list_channels_ai_talk=list_channels_ai_talk,
        )
        db.insert_guild_extra_info(data)
        await interaction.followup.send(
            f"Đã tạo Guild Extra Info. Bot lâu lâu sẽ nói chuyện trong channel này.",
            ephemeral=True,
        )
...
async def automatic_speak_randomly_true_heaven():
    ...
    if guild is None:
        return  # Bot not in that guild
    guild_extra_info = db.find_guild_extra_info_by_id(guild.id)
    if (
        guild_extra_info != None
        and guild_extra_info.list_channels_ai_talk != None
        and len(guild_extra_info.list_channels_ai_talk) > 0
    ):
        random_channel_id = random.choice(guild_extra_info.list_channels_ai_talk)
        actual_channel = guild.get_channel(random_channel_id)
        if actual_channel:
            try:
                prompt = CustomFunctions.get_automatically_talk_prompt(
                    "Creation 2", guild, actual_channel
                )
                completion = groq_client.chat.completions.create(
                    model=CustomFunctions.AI_MODEL,
                    messages=[{"role": "user", "content": prompt}],
                )
                bot_response = CustomFunctions.remove_creation_name_prefix(
                    f"{completion.choices[0].message.content}"
                )
                # response = model.generate_content(f"{prompt}")
                bot_response = bot_response.replace("@everyone", "")
                print(
                    f"{bot.user} started talking on its own at {guild_extra_info.guild_name}, channel {actual_channel.name}."
                )
                async with actual_channel.typing():
    ...
    if list_all_user_convo_info:
        for data in list_all_user_convo_info:
            if data.last_time_interaction < three_day_before:
                ...
                count += 1
    print(
        f"Found {count} old conversation in collection 'user_conversation_info_{bot_name}' and deleted them."
    )
...
async def random_dropbox():
    ...
    for guild in guilds:
        ...
        if guild_info == None:
            continue
        if (
            guild_info.list_channels_quests == None
            or len(guild_info.list_channels_quests) <= 0
        ):
            continue
        ...
        if quest_channel != None:
            ...
            embed = discord.Embed(
                title=f"",
                description=f"{EmojiCreation2.GOLDEN_GIFT_BOX.value} **Hộp Quà Thần Bí** {EmojiCreation2.GOLDEN_GIFT_BOX.value}",
                color=0x0CE7F2,
            )
            embed.add_field(name=f"", value="▬▬▬▬ι══════════>", inline=False)
            embed.add_field(
                name=f"",
                value=f"{EmojiCreation2.SHINY_POINT.value} Một hộp quà thần bí đã xuất hiện tại đúng channel này!",
                inline=False,
            )
            embed.add_field(
                name=f"",
                value=f"{EmojiCreation2.SHINY_POINT.value} Ai nhanh tay thì được nhé, vì hộp quà sẽ biến mất đúng sau: <t:{int(endtime.timestamp())}:R>",
                inline=False,
            )
            embed.add_field(name=f"", value="▬▬▬▬ι══════════>", inline=False)
            embed.set_footer(
                text=f"Hộp quà sẽ xuất hiện ngẫu nhiên, và khi thấy thì nhớ nhanh tay nhé!",
                icon_url=f"{EmojiCreation2.TRUE_HEAVEN_LINK_MINI.value}",
            )
            print(
                f"Created random dropbox at channel {quest_channel.name} in guild {guild.name}."
            )
...
async def random_quizz_embed():
    ...
    for guild in guilds:
        ...
        if quest_channel != None:
            ...
            embed = discord.Embed(
                title=f"",
                description=f"{EmojiCreation2.QUESTION_MARK.value} **Hỏi Nhanh Có Thưởng** {EmojiCreation2.QUESTION_MARK.value}",
                color=0x0CE7F2,
            )
            embed.add_field(name=f"", value="▬▬▬▬ι════════>", inline=False)
            embed.add_field(
                name=f"", value=f"**Câu hỏi**: {random_quizz.question}", inline=False
            )
            ...
            embed.add_field(
                name=f"",
                value=f"Nhanh tay lên nhé, vì câu hỏi sẽ biến mất sau: <t:{int(endtime.timestamp())}:R>",
                inline=False,
            )
            embed.set_footer(
                text=f"Hỏi Nhanh Có Thưởng sẽ xuất hiện ngẫu nhiên, và khi thấy thì nhớ trả lời đúng nhé!",
                icon_url=f"{EmojiCreation2.TRUE_HEAVEN_LINK_MINI.value}",
            )
            print(
                f"Created random quizz at channel {quest_channel.name} in guild {guild.name}."
            )
            m = await quest_channel.send(embed=embed, view=view)
            view.old_message = m
    ...
    for guild in guilds:
        ...
        for couple in couples_in_guild:
            if couple.love_point <= 0:
                ...
                if calculated_love_progressing <= 0:
                    calculated_love_progressing = 0
                CoupleMongoManager.set_love_progressing_value(
                    guild_id=guild.id,
                    user_id=couple.first_user_id,
                    love_progressing=calculated_love_progressing,
                )
            ...
            if couple.love_point <= 0:
                couple.love_point = 0
            CoupleMongoManager.set_love_point_value(
                guild_id=guild.id,
                user_id=couple.first_user_id,
                love_point=couple.love_point,
            )
            if couple.love_rank == 20:
                continue
            two_weeks_ago = datetime.now() - timedelta(weeks=2)
            two_weeks_after = datetime.now() + timedelta(weeks=2)
            delete_check = 0
            if couple.last_fight_action == None and couple.last_love_action == None:
                delete_check += 1
            if (
                couple.last_love_action != None
                and couple.last_love_action + timedelta(weeks=2) < datetime.now()
            ):
                delete_check += 1
            if (
                couple.last_fight_action != None
                and couple.last_fight_action + timedelta(weeks=2) < datetime.now()
            ):
                delete_check += 1
            if couple.love_point == 0:
                delete_check += 1
            if couple.love_point == 0 and couple.love_progressing == 0:
                delete_check += 1
            # Mới tạo trong vòng 2 tuần không cần check
            if couple.date_created + timedelta(weeks=2) > datetime.now():
                delete_check -= 2
            if couple.love_rank == 20:
                delete_check = -99
            # Nếu đạt trên ba tiêu chí trên thì xoá
            if delete_check >= 3:
                CoupleMongoManager.delete_couple_by_id(
                    guild_id=guild.id, user_id=couple.first_user_id
                )
                print(
                    f"Check delete for couple id {couple.first_user_id} reached. Is deleted"
                )
...
async def clear_up_data_task():
    ...
    if all_global_inventories:
        for global_inventory in all_global_inventories:
            if global_inventory.date_updated + timedelta(weeks=24) < datetime.now():
                # Xóa
                GlobalMongoManager.delete_global_item_by_user_id(
                    user_id=global_inventory.user_id
                )
                count += 1
    print(
        f"clear_up_data_task started. Deleted {count} global inventory due to in-active"
    )
    ...
    if all_quest_collection:
        for guild_id in all_quest_collection:
            all_quest_data = QuestMongoManager.find_all_quest_profiles(
                guild_id=guild_id
            )
            count = 0
            if all_quest_data:
                for quest in all_quest_data:
                    if datetime.now() > quest.reset_date:
                        QuestMongoManager.delete_quest(
                            guild_id=guild_id, user_id=quest.user_id
                        )
                        count += 1
            QuestMongoManager.drop_quest_collection_if_empty(guild_id=guild_id)
            print(
                f"clear_up_data_task started. Deleted {count} quest data in guild {guild_id}"
            )
        ...
        for guild_id in list_guilds_id_snipe:
            ...
            if all_snipe_channels != None:
                ...
                for snipe_channel in all_snipe_channels:
                    if (
                        snipe_channel.snipe_messages != None
                        and len(snipe_channel.snipe_messages) > 0
                    ):
                        # Xóa bớt message
                        snipe_messages = snipe_channel.snipe_messages
                        filtered_messages = [
                            msg
                            for msg in snipe_messages
                        ...
                        db.replace_snipe_message_info(
                            guild_id=guild_id,
                            channel_id=snipe_channel.channel_id,
                            snipe_messages=snipe_messages,
                        )
                    else:
                        # Xóa channel
                        db.delete_snipe_channel_info(
                            guild_id=guild_id, channel_id=snipe_channel.channel_id
                        )
            print(
                f"clear_up_data_task started. Deleted {count} snipe message in {guild_id}"
            )
        ...
        for guild_id in list_guilds_id_user_count:
            ...
            if all_user_count:
                for user_count in all_user_count:
                    if (
                        user_count.last_interaction + timedelta(weeks=3)
                        < datetime.now()
                    ):
                        # Xóa
                        db.delete_user_count(
                            guild_id=guild_id, user_id=user_count.user_id
                        )
                        count += 1
            print(
                f"clear_up_data_task started. Deleted {count} user count in {guild_id} due to in-active"
            )
...
async def dungeon_spawn_enemy_embed():
    ...
    for guild in guilds:
        ...
        if (
            is_inside_disable_time
            and guild.id != TrueHeavenEnum.TRUE_HEAVENS_SERVER_ID.value
        ):
            return
        if (
            CustomFunctions.check_if_dev_mode() == True
            and guild.id != TrueHeavenEnum.TRUE_HEAVENS_SERVER_ID.value
            and guild.id != 1293554240593330241
        ):
            continue
        # Kiểm tra quest channel của server, nếu có thì mới chọn
        guild_info = db.find_guild_extra_info_by_id(guild_id=guild.id)
        if guild_info == None:
            continue
        if (
            guild_info.list_channels_dungeon == None
            or len(guild_info.list_channels_dungeon) <= 0
        ):
            continue
        ...
        if guild.id == TrueHeavenEnum.TRUE_HEAVENS_SERVER_ID.value:
            for quest_channel in list_channels_dungeon:
                await spawning_enemy_embed_in_dungeon(
                    guild=guild,
                    random_quest_channel_id=quest_channel,
                    list_channels_dungeon=list_channels_dungeon,
                )
                await asyncio.sleep(5)
        else:
            await spawning_enemy_embed_in_dungeon(
                guild=guild,
                random_quest_channel_id=random_quest_channel_id,
                list_channels_dungeon=list_channels_dungeon,
            )


async def spawning_enemy_embed_in_dungeon(
    guild: discord.Guild,
    random_quest_channel_id: DungeonQuestChannel,
    ...
    if quest_channel == None:
        ...
        list_channels_dungeon = [
            c
            for c in list_channels_dungeon
            if c.channel_id != random_quest_channel_id.channel_id
        ]
        db.update_guild_extra_info_list_channels_dungeon(
            guild_id=guild.id, list_channels_dungeon=list_channels_dungeon
        )
        return
    if quest_channel != None:
        ...
        embed = discord.Embed(
            title=f"",
            description=f"{EmojiCreation2.STUN_SKILL.value} **Hầm Ngục {UtilitiesFunctions.get_cap_do_quest(random_quest_channel_id.difficulty_level)}** {EmojiCreation2.STUN_SKILL.value}",
            color=0x0CE7F2,
        )
        embed.add_field(name=f"", value="▬▬▬▬ι════════>", inline=False)

        double_enemy_dice = UtilitiesFunctions.get_chance(double_enemy_chance)
        enemy = ListGAAndSkills.get_random_ga_enemy_generic(
            level=level, guardian_chance=guardian_chance
        )
        enemy_2 = None

        if double_enemy_dice:
            enemy = ListGAAndSkills.get_random_ga_enemy_generic(
                level=int(level / 2), guardian_chance=guardian_chance
            )
        ...
        print(
            f"Spawning enemy with base level around {level} at channel {quest_channel.name} in guild {guild.name}. Difficult dungeon: {random_quest_channel_id.difficulty_level}"
        )
        view = GaDugeonView(
            guild_id=guild.id,
            enemy_ga=enemy,
            enemy_ga_2=enemy_2,
            title=f"{EmojiCreation2.STUN_SKILL.value} **Hầm Ngục {UtilitiesFunctions.get_cap_do_quest(random_quest_channel_id.difficulty_level)}** {EmojiCreation2.STUN_SKILL.value}",
            bonus_percent=bonus_percent,
            difficulty=random_quest_channel_id.difficulty_level,
            footer_text=footer_text,
            channel_name=quest_channel.name,
        )
        m = await quest_channel.send(embed=embed, view=view)
        view.message = m
...
async def on_message(message: discord.Message):
    ...
    await ai_handling_response.sub_function_ai_response(
        message=message, speakFlag=speakFlag
    )
...
async def on_message_delete(message):
    ...
    if (
        message.guild.id == TrueHeavenEnum.TRUE_HEAVENS_SERVER_ID.value
        and message.attachments != None
        ...
        embed = discord.Embed(
            title=f"Một tin nhắn đã bị xoá trong server {message.guild.name}",
            description=f"Tin nhắn của {message.author.mention} đã bị xoá tại {channel_where_message_deleted.mention}!",
            color=0xFC0345,
        )
        embed.add_field(
            name="Nội dung tin nhắn bị xoá:", value=message.content, inline=False
        )
        embed.add_field(
            name=f"Tin nhắn chứa {len(message.attachments)} Attachments.",
            value="",
            inline=False,
        )
        for index, attachment in enumerate(message.attachments):
            embed.add_field(
                name="", value=f"{index + 1}. {attachment.url}", inline=False
            )
            file = await CustomFunctions.get_attachment_file_from_url(
                url=attachment.url, content_type=attachment.content_type
            )
            if file != None:
    ...
    if message.guild and message.author.bot != True:
        ...
        if message.attachments:
            for att in message.attachments:
                new_url = att.url
                data_attachmenta = db.SnipeMessageAttachments(
        ...
        existing_snipe_channel_info = db.find_snipe_channel_info_by_id(
            channel_id=channel_where_message_deleted.id, guild_id=message.guild.id
        )
        if existing_snipe_channel_info == None:
            list_temp = []
            list_temp.append(snipe_message)
            existing_snipe_channel_info = db.SnipeChannelInfo(
                channel_id=channel_where_message_deleted.id,
                channel_name=channel_where_message_deleted.name,
                snipe_messages=list_temp,
            )
            result = db.create_snipe_channel_info(
                snipe_channel_info=existing_snipe_channel_info,
                guild_id=message.guild.id,
            )
            print(
                f"Successfully create new Snipe Channel Info for guild {message.guild.name}"
            )
        else:
            # Cập nhật snipe_messages của SnipeChannelInfo ấy
            result = db.update_or_insert_snipe_message_info(
                guild_id=message.guild.id,
                channel_id=channel_where_message_deleted.id,
...
async def on_reaction_add(reaction, user):
    ...
    if user_target != None and message != None:
        check_quest_message = QuestMongoManager.increase_emoji_count(
            guild_id=user_target.guild.id,
        ...
        if check_quest_message == True:
            ...
            m = await message.channel.send(
                embed=quest_embed, view=view, content=f"{user.mention}"
            )
            view.message = m
    return
...
async def on_member_remove(member: discord.Member):
    ...
    if profile != None:
        ...
        print(
            f"Member {member.name} left server {guild.name} so their economy profile is deleted!"
        )
    quest = QuestMongoManager.find_quest_by_user_id(
        guild_id=guild.id, user_id=member.id
    )
    if quest != None:
        ...
        print(
            f"Member {member.name} left server {guild.name} so their quest is deleted!"
        )
    ...
    if couple != None:
        ...
        print(
            f"Member {member.name} left server {guild.name} so their couple is deleted!"
        )
...
async def on_guild_remove(guild: discord.Guild):
    ...
    print(
        f"Bot {bot.user.display_name} removed from guild {guild.name}. Deleted all related collection"
    )
...
async def on_member_update(before: discord.Member, after: discord.Member):
    ...
    if before.guild.id != TrueHeavenEnum.TRUE_HEAVENS_SERVER_ID.value:
        return
            ...
            try:
                for role in roles_to_remove:
                    ...
                    try:
                        ...
                        print(
                            f"Removed role '{role.name}' from {after.name} due to 'Đáy Xã Hội' restriction."
                        )
                    except Exception:
                        continue
...
# Cog command
init_extension = [
    "cogs.games.RockPaperScissorCog",
    "cogs.games.TruthDareCog",
    "cogs.games.CoinFlipCog",
    "cogs.games.SicBoCog",
    "cogs.games.DuaNguaCog",
