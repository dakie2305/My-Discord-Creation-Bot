...
class CoinFlip(commands.Cog):
    ...
    @discord.app_commands.command(
        name="cf", description="Tung đồng xu sấp/ngửa cho vui"
    )
    ...
    async def coin_flip_slash_command(
        self,
        ...
        if (
            ...
            embed = discord.Embed(
                title=f"Darkie đang nghiên cứu, cập nhật và sửa chữa bot! Vui lòng đợi nhé!",
        ...
        embed = discord.Embed(
            title=f"",
            description=f"{interaction.user.mention} đã tung đồng xu. Đồng xu đang quay {EmojiCreation2.DOGE_COIN.value} ...",
            color=0x03F8FC,
        )
        mess = await interaction.followup.send(embed=embed)
        if mess:
            if sap_ngua != None and sap_ngua == "n":
                sap_ngua_true_false = False
            elif sap_ngua != None and sap_ngua == "s":
                sap_ngua_true_false = True
            else:
                sap_ngua_true_false = None
            await self.edit_embed_coin_flip(
                message=mess,
                user=interaction.user,
                sap_ngua=sap_ngua_true_false,
                so_tien=so_tien,
                loai_tien=loai_tien,
                profile=profile,
            )
    ...
    async def coin_flip_slash_command_error(
    ...
    async def edit_embed_coin_flip(
        ...
        embed_updated = discord.Embed(
            title=f"",
            description=f"{user.mention} đã tung đồng xu. Đồng xu đã quay ra **`{state}`** {emoji_state}!{gambling_text}",
            color=0x03F8FC,
        )
        await message.edit(embed=embed_updated)
        if choice == 0:
            ...
            embed_updated = discord.Embed(
                title=f"",
                description=f"Đùa thôi. Đồng xu đã quay ra **`{state}`** {emoji_state}!{gambling_text}",
                color=0x03F8FC,
            )
            await message.edit(embed=embed_updated)
        ...
        if sap_ngua != None and profile != None:
            ...
            authority_profile = ProfileMongoManager.get_authority(
            ...
            if loai_tien == "C":
                if is_player_win:
                    ...
                    if authority_profile and profile.is_authority == False:
                        authority_profile.copper -= so_tien
                else:
                    profile.copper -= so_tien
                    if authority_profile and profile.is_authority == False:
                        au_money_tax = int(so_tien / 2)
            ...
            if loai_tien == "S":
                if is_player_win:
                    ...
                    if authority_profile and profile.is_authority == False:
                        authority_profile.silver -= so_tien
                else:
                    profile.silver -= so_tien
                    if authority_profile and profile.is_authority == False:
                        au_money_tax = int(so_tien / 2)
            ...
            if loai_tien == "G":
                if is_player_win:
                    ...
                    if authority_profile and profile.is_authority == False:
                        authority_profile.gold -= so_tien
                else:
                    profile.gold -= so_tien
                    if authority_profile and profile.is_authority == False:
                        au_money_tax = int(so_tien / 2)
            ...
            # Cộng kinh nghiệm nếu thắng
                ...
                if loai_tien == "C" and so_tien > 10000:
                    bonus_exp = self.get_bonus_exp_based_on_amount(
                        so_tien=so_tien, loai_tien=loai_tien, profile=profile
                    )
                    ProfileMongoManager.update_level_progressing(
                        guild_id=user.guild.id, user_id=user.id, bonus_exp=bonus_exp
                    )
            ...
            ProfileMongoManager.update_profile_money_fast(
                guild_id=user.guild.id, data=profile
            )
            if authority_profile and profile.is_authority == False:
                ProfileMongoManager.update_profile_money_fast(
                    guild_id=user.guild.id, data=authority_profile
                )
        ...
        if check_quest_message == True:
            ...
            ms = await message.channel.send(
                embed=quest_embed, content=f"{user.mention}", view=view
            )
            view.message = ms
        return

    def get_bonus_exp_based_on_amount(
        self, so_tien: int = None, loai_tien: str = None, profile: Profile = None
    ):
        if profile == None or so_tien == None or loai_tien == None:
            return 0
        if so_tien < 10000 and loai_tien == "C":
            return 0
        if loai_tien == "C":
