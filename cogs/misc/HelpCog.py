...
class Help(commands.Cog):
    ...
    def get_list_help_creation_2(self):
        ...
        count+=1
        embed.add_field(name=f"", value=f"- **Trang {count}**: Chức năng phụ khác.", inline=False)
        count+=1
        ...
        embed.add_field(name=f"", value=f"{EmojiCreation2.SHINY_POINT.value}`/keo_bua_bao`: chơi game kéo búa bao, có thể đặt cược tiền.", inline=False)
        ...
        embed.add_field(name=f"", value=f"{EmojiCreation2.SHINY_POINT.value}`/work normal`: làm việc để kiếm tiền và nhân phẩm. Mỗi một tiếng được dùng lệnh một lần.", inline=False)
        ...
        embed.add_field(name=f"", value=f"{EmojiCreation2.SHINY_POINT.value}`/inventory attack`: sẽ dùng vũ khí đang mang trong profile để tấn công người khác!", inline=False)
        ...
        embed.add_field(name="", value=f"{EmojiCreation2.SHINY_POINT.value}Kho Đồ Liên Thông ({SlashCommand.GLOBAL_INVENTORY.value}) tức là kho đồ chung, bạn có thể truy cập từ bất kỳ server nào! Khi đủ điều kiện liên thông thì bạn có thể bỏ tối đa **10** vật phẩm từ kho đồ cá nhân để đem sang server khác", inline=False)
        embed.add_field(name="", value=f"{EmojiCreation2.SHINY_POINT.value}Liên Thông Hộ Vệ Thần ({SlashCommand.GLOBAL_GA.value}) là chức năng cho phép bạn có thể đồng bộ dữ liệu Hộ Vệ Thần của bản thân ở server khác!", inline=False)
        ...
        embed.add_field(name="", value=f"{EmojiCreation2.SHINY_POINT.value}Để được phép liên thông từ server -> global, cần phải đáp ứng điều kiện: server trên 1500 người, hoặc dùng lệnh trong server True Heavens!", inline=False)
        ...
        embed.set_footer(text=f"{EmojiCreation2.SHINY_POINT.value} Lưu ý, vật phẩm và Hộ Vệ Thần đã liên thông sẽ mất tất cả giá trị bán ra, nên tốt nhất đừng mơ đến chuyện đem ra server khác bán!", icon_url=f"{EmojiCreation2.TRUE_HEAVEN_LINK_MINI.value}")
        embed.add_field(name=f"", value="▬▬▬▬ι═════════>", inline=False)
        list_embed.append(embed)
        ...
        embed.add_field(name=f"", value=f"{EmojiCreation2.SHINY_POINT.value}`/authority overthrow`: lật đổ Chính Quyền hiện tại! Chỉ dành cho chủ server hoặc Chính Quyền hiện tại! Phạt tiền chính quyền cực nặng!", inline=False)
        ...
        embed.add_field(name=f"", value=f"- Làm được hai tuần có thể từ chức trong hòa bình và giữ được tiền!", inline=False)
        ...
        embed.add_field(name="", value=f"- Có thể dùng Hộ Vệ Thần của mình trong server khác. Hãy nhắn `global help` để tìm hiểu cách Liên Thông Hộ Vệ Thần!", inline=False)
        ...
        embed.add_field(name="", value=f"- Dùng lệnh {SlashCommand.GA_QUEST.value} để làm nhiệm vụ cốt truyện nho nhỏ và nhận thưởng thêm!", inline=False)
        embed.add_field(name="", value=f"- Dùng Ác Linh Sách để triệu hồi boss Hộ Vệ Thần cực mạnh với phần thưởng cao!", inline=False)
        ...
        #Misc
        embed = discord.Embed(title=f"Các lệnh phụ khác", description=f"", color=0xddede7)
        embed.add_field(name=f"", value="▬▬▬▬ι═════════>", inline=False)
        embed.add_field(name=f"", value=f"{EmojiCreation2.SHINY_POINT.value} Có thể xem lại tin nhắn đã xóa bằng `/snipe` !", inline=False)
        embed.add_field(name=f"", value=f"{EmojiCreation2.SHINY_POINT.value} Có thể chọn ngẫu nhiên bằng cú pháp `= A, B, C`!", inline=False)
        embed.add_field(name=f"", value=f"{EmojiCreation2.SHINY_POINT.value} Có thể xem bảng xếp hạng bằng lệnh `/leaderboard`!", inline=False)
        embed.add_field(name=f"", value=f"{EmojiCreation2.SHINY_POINT.value} Có thể chơi Truth Dare bằng lệnh `/truth_dare`!", inline=False)
        list_embed.append(embed)
