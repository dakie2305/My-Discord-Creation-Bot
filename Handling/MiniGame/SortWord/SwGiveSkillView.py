...
class SwGiveSkillView(discord.ui.View):
    def __init__(self, user: discord.Member, target: discord.Member, channel_id: str, lan: str, list_embed: list[discord.Embed], all_skills: list[SwClass.SwSpecialItem]):
...
class GiveSkillInputModal(discord.ui.Modal):
    def __init__(self, user: discord.Member, target: discord.Member, channel_id: str, lan: str, list_skills: list[SwClass.SwSpecialItem]):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if chosen_skill is None:
                ...
                return
            SwMongoManager.update_player_special_item(channel_id=self.channel_id, guild_id=interaction.guild_id, language=self.lan, user_id=self.target.id, user_display_name=self.target.display_name, user_name=self.target.name, point=0, special_item=chosen_skill)
