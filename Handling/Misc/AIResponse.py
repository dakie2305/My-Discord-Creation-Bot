...
class AIResponseHandling:
    def __init__(self, bot: commands.Bot, key: str):
        self.bot = bot
        self.bot_name = (
            "Creation 2" if bot.user.id == UserId.CREATION_2.value else "Creation 1"
        )
        self.bot_to_bot_history = deque()
        self.user_to_bot_history = deque()
        self.groq_client = Groq(api_key=key)

    async def sub_function_ai_response(
        self, message: discord.Message, speakFlag: bool = True
    ):
        if speakFlag == False:
            return
    ...
    async def generate_ai_response(self, message: discord.Message):
        ...
        if (
            message.author.id in [UserId.CREATION_1.value, UserId.CREATION_2.value]
            and message.author.id != self.bot.user.id
        ):
            now = datetime.datetime.now()
            self.bot_to_bot_history = deque(
                ts
                for ts in self.bot_to_bot_history
            ...
            self.bot_to_bot_history = deque(
                ts
                for ts in self.bot_to_bot_history
                if now - ts < datetime.timedelta(minutes=3)
            )
            self.bot_to_bot_history.append(now)
            if len(self.bot_to_bot_history) >= 15:
                ...
                await message.channel.send(
                    f"Vui lòng không spam chức năng A.I. của bot quá nhanh."
                )
                await self.alert_creator(text)
                return
        ...
        bots_creation_name = (
            bots_creation_2_name
            if self.bot.user.id == UserId.CREATION_2.value
            else bots_creation_1_name
        )
        ...
        if message.reference is not None and message.reference.resolved is not None:
            if (
                ...
                or CustomFunctions.contains_substring(
                    message.content.lower(), bots_creation_name
                )
            ):
            ...
            if (
                ...
                and message.guild.id != 1194106864582004849
            ):  # Chỉ True Heaven, Học Viện 2ten mới không bị dính
                if CustomFunctions.is_inside_working_time() == False:
                    await message.channel.send(
                        f"Tính năng AI của Bot chỉ hoạt động đến 12h đêm, vui lòng đợi đến 8h sáng hôm sau."
                    )
                    return
            elif (
                message.guild.id == 1194106864582004849
                and message.channel.id != 1381630185723531264
            ):  # Học viện #sân-chơi-creation-2
                return
            is_reply_message = False
        ...
        if flag != 0:
            ...
            print(
                f"Username {message.author.name}, Display user name {message.author.display_name} violated chat when talking to {self.bot.user}"
            )
            return
        ...
        try:
            async with message.channel.typing():
                ...
                prompt = await CustomFunctions.get_proper_prompt(
                    message, self.bot_name, referenced_message
                )
                print(f"Prompt generated from {self.bot.user}:\n {prompt}")
                # response = model.generate_content(f"{prompt}")

                # Groq Chat Completion Call
                bot_response = await self.fetch_and_sanitize_ai_response(
                    system_instruction, prompt
                )
                # Nếu có chữ record thì tạo file và gửi ghi âm
                if "record" in message.content.lower():
                    await CustomFunctions.bot_sending_sound(
                        bot_name="Creation_2", bot_reponse=bot_response, message=message
                    )
                    print(
                        f"Username {message.author.name}, Display user name {message.author.display_name} tell {self.bot.user} to send record"
                    )
                    return

                # Nếu là bot thì đương nhiên không reply, chỉ nhắn bình thường thôi
                if (
                    message.author.id == CustomFunctions.user_cr_1["user_id"]
                    or message.author.id == CustomFunctions.user_cr_2["user_id"]
                    or message.author.id == CustomFunctions.user_cr_3["user_id"]
                ):
                    await message.channel.send(
                ...
                CustomFunctions.save_user_convo_data(
                    message=message, bot_reponse=bot_response, bot_name=self.bot_name
                )
                print(
                    f"Username {message.author.name}, Display user name {message.author.display_name} replied {self.bot.user}"
                )
                return
        except exceptions.ResourceExhausted as e:
            # "Out of Quota" / Rate Limit error
            await message.reply(
                "Cảm ơn bạn đã đồng hành cùng bot suốt thời gian qua. Google đã không còn cho sử dụng AI miễn phí nữa, nên coi như tính năng A.I. thông minh của bot đến đây là kết thúc.\nMong một ngày bot trở về, và nếu không thì đành thôi vậy."
            )
        except Exception as e:
            print(
                f"Username {message.author.name}, Display user name {message.author.display_name} replied {self.bot.user} but with error: {e}"
            )
        return
    ...
    async def handling_therapy_ai(self, message: discord.Message):
        ...
        ) and not any(name in message.content.lower() for name in bots_creation1_name):
            return
        if (
            message.guild.id != TrueHeavenEnum.TRUE_HEAVENS_SERVER_ID.value
            and message.guild.id != 1194106864582004849
        ...
        async with message.channel.typing():
            if message.reference is not None and message.reference.resolved is not None:
                if (
                    message.reference.resolved.author == self.bot.user
                    or CustomFunctions.contains_substring(
                        message.content.lower(), bots_creation1_name
                    )
                ):
            ...
            # Lấy prompt
            prompt = await CustomFunctions.get_therapy_prompt(
                message=message, extra_message=ref_message
            )
            print(prompt)
            bot_response = await self.fetch_and_sanitize_ai_response(
