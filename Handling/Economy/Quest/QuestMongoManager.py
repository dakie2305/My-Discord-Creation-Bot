...
if (
    UtilitiesFunctions.USER_NAME_MONGODB != ""
    and UtilitiesFunctions.USER_NAME_MONGODB != None
    and UtilitiesFunctions.PASSWORD_MONGODB != ""
    and UtilitiesFunctions.PASSWORD_MONGODB != None
):
    client = MongoClient(
        f"mongodb://{UtilitiesFunctions.USER_NAME_MONGODB}:{UtilitiesFunctions.PASSWORD_MONGODB}@localhost:27017/"
    )
else:
...
def create_new_random_quest(
    ...
    if quest_type == "emoji_reaction_count":
        ...
        if reward_type == "C":
            ...
            base_reward_amount = 10 * rand_reward_amount
        quest_title = (
            f"Thả **{base_amount}** ảnh, video bất kỳ vào kênh <#{channel_id}>"
        )
        quest_des = f"**{base_reward_amount}**{emoji}"
    elif quest_type == "truth_game_count" or quest_type == "dare_game_count":
        game_name = "Truth"
        if quest_type == "dare_game_count":
            game_name = "Dare"
        ...
        if reward_type == "C":
            ...
            base_reward_amount = 10 * rand_reward_amount
        quest_title = (
            f"Chơi **{base_amount}** trận game Tung Xu ({SlashCommand.COIN_FLIP.value})"
        )
        quest_des = f"**{base_reward_amount}**{emoji}"
        ...
        quest_title = (
            f"Câu cá **{base_amount}** lần ({SlashCommand.WORK_FISHING.value})"
        )
        quest_des = f"**{base_reward_amount}**{emoji}"
        ...
        quest_title = (
            f"Trồng cây **{base_amount}** lần ({SlashCommand.WORK_PLANTING.value})"
        )
        quest_des = f"**{base_reward_amount}**{emoji}"
        ...
        if reward_type == "C":
            ...
            base_reward_amount = 5
        quest_title = (
            f"Tặng quà **{base_amount}** lần cho người khác ({SlashCommand.GIFT.value})"
        )
        quest_des = f"**{base_reward_amount}**{emoji}"
    ...
    elif quest_type == "dua_ngua_count":
        base_amount = quest_difficult_rate * 1
        rand_reward_amount = random.randint(1, 5)
        base_reward_amount = 55500
        bonus_exp += rand_reward_amount * 50
        if reward_type == "C":
            ...
            base_reward_amount = 150 * rand_reward_amount
        elif reward_type == "G":
            base_reward_amount = 50 * rand_reward_amount
        quest_title = f"Chơi mini game đua ngựa **{base_amount}** lần ({SlashCommand.DUA_NGUA.value})"
        quest_des = f"**{base_reward_amount}**{emoji}"
    ...
    quest_profile = QuestProfile(
        ...
        quest_channel=channel_id,
        channel_name=channel_name,
        quest_title=quest_title,
        quest_description=quest_des,
        quest_difficult_rate=quest_difficult_rate,
        quest_total_progress=base_amount,
        bonus_exp=bonus_exp,
    )
    if reward_type == "C":
...
def increase_truth_dare_count(guild_id: int, user_id: int, is_truth: bool):
    ...
    data = collection.find_one(
        {"id": "quest", "user_id": user_id, "quest_type": quest_type}
    )
    if data == None:
        return False
    ...
    collection.update_one(
        {"id": "quest", "user_id": user_id},
        {
            "$set": {
                "truth_game_count": existing_data.truth_game_count,
...
def increase_coin_flip_count(guild_id: int, user_id: int):
    collection = db_specific[f"quest_{guild_id}"]
    data = collection.find_one(
        {"id": "quest", "user_id": user_id, "quest_type": "coin_flip_game_count"}
    )
    if data == None:
        return False
    existing_data = QuestProfile.from_dict(data)
...
def increase_sb_normal_count(guild_id: int, user_id: int):
    collection = db_specific[f"quest_{guild_id}"]
    data = collection.find_one(
        {"id": "quest", "user_id": user_id, "quest_type": "sb_normal_count"}
    )
    if data == None:
        return False
    existing_data = QuestProfile.from_dict(data)
...
def increase_sb_double_count(guild_id: int, user_id: int):
    collection = db_specific[f"quest_{guild_id}"]
    data = collection.find_one(
        {"id": "quest", "user_id": user_id, "quest_type": "sb_double_count"}
    )
    if data == None:
        return False
    existing_data = QuestProfile.from_dict(data)
...
def increase_sb_triple_count(guild_id: int, user_id: int):
    collection = db_specific[f"quest_{guild_id}"]
    data = collection.find_one(
        {"id": "quest", "user_id": user_id, "quest_type": "sb_triple_count"}
    )
    if data == None:
        return False
    existing_data = QuestProfile.from_dict(data)
...
def increase_rps_count(guild_id: int, user_id: int):
    collection = db_specific[f"quest_{guild_id}"]
    data = collection.find_one(
        {"id": "quest", "user_id": user_id, "quest_type": "rps_game_count"}
    )
    if data == None:
        return False
    existing_data = QuestProfile.from_dict(data)
    ...
    data = collection.find_one(
        {
            "id": "quest",
            "user_id": user_id,
            "quest_type": "emoji_reaction_count",
            "quest_channel": channel_id,
        }
    )
    if data == None:
...
def increase_quest_objective_count(
    guild_id: int, user_id: int, quest_type: str, channel_id: int = None
):
    collection = db_specific[f"quest_{guild_id}"]
    if channel_id != None:
        data = collection.find_one(
