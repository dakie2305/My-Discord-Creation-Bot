...
class GaDugeonView(discord.ui.View):
    ...
    async def on_timeout(self):
        #Delete
        if self.message is not None and self.is_attacked == False:
            try:
    ...
    def create_embed(self, selected_user_profile: Profile, selected_user: discord.Member):
        ...
        if self.difficulty >= 3 and not self.is_top_1_server:
            ...
            scale_level = selected_user_profile.guardian.level + 10
            if self.difficulty >= 4:
                scale_level = max(scale_level, 110) # Độ khó trên 4 thì cấp tối thiểu là 110
            
            if self.enemy_ga.level < selected_user_profile.guardian.level:
